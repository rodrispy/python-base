...
# Vamos usar fator 0.1 (10% da lista será para treino e o resto para teste)
# Definindo o índice de corte que dividirá a lista:

fator = 0.1
i_corte = int((1 - fator) * len(precos_imoveis))
precos_treino = precos_imoveis[:i_corte]
precos_teste = precos_imoveis[i_corte:]
...
def separar_listas(precos, tamanhos, fator=0.1):
    if len(precos) == len(tamanhos):
        ...
        return precos_imoveis_teste, precos_imoveis_treino, tamanho_imoveis_teste, tamanho_imoveis_treino
    else:
        print('As listas não tem a mesma quantidade de itens')

precos_treino, precos_teste, tamanhos_treino, tamanhos_teste = separar_listas(precos_imoveis, tamanho_imoveis)
print(precos_treino, precos_teste, tamanhos_treino, tamanhos_teste, sep='\n')