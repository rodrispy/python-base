...
for estado in niveis_co2:
    soma = sum(niveis_co2[estado])
    media = soma/len(niveis_co2[estado])
    
    if media > nivel_alerta:
        print(f'O estado {estado} está com níveis altíssimos de CO2 ({media})')
...
# Para resolução, o ideal é ir printando os itens do dicionário para entender como ele está estruturado e o que podemos aproveitar de cada informação:
for item in video:
    print(item)
    
# Agora podemos pegar a informação "download" e desdobrá-la para encontrarmos o que precisamos:
print(video['download'])

# A chave 'download' é uma lista de dicionários. Podemos desdobrar ainda mais:
print(video['download'][0])

# download[0] é um dicionário: podemos pegar a chave 'link' desse dicionário, que é o que nos importa
